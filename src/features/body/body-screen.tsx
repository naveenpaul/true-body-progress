...
import { today, formatRelativeDate } from '@/lib/dates';
...
export function BodyScreen() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const weight = weightInput ? parseWeightInput(parseFloat(weightInput), units) : undefined;
    const waist = waistInput ? parseLengthInput(parseFloat(waistInput), units) : undefined;
    const bodyFat = bodyFatInput ? parseFloat(bodyFatInput) : undefined;

    await store.logMetric(today(), weight, waist, bodyFat, notesInput || undefined);
    setWeightInput('');
    setWaistInput('');
    setBodyFatInput('');
    setNotesInput('');
        ...
        <View className="mb-6">
          ...
          {recentMetrics.map(metric => (
            <View key={metric.id} className="flex-row justify-between border-b border-charcoal-800 py-3">
              ...
              {metric.body_fat != null && <Text className="ml-4 text-sm text-charcoal-400">{metric.body_fat}%</Text>}
            </View>
          ))}
        </View>
      )}
